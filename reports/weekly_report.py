...
def generate_weekly_report():
    ...
    filepath = f"reports/output/weekly_report_{today}.pdf"
    ...
    # Encabezado
    story.append(Paragraph("Reporte Semanal de Movimientos", title_style))
    story.append(Paragraph(f"Semana: {start_week} a {today}", normal_style))
    story.append(Spacer(1, 40))
    story.append(Paragraph(
        "Este reporte presenta los eventos registrados en el sistema de monitoreo de personas, durante la semana actual.",
        normal_style
    ))
    ...
    cur = conn.cursor()
    cur.execute("SELECT id, action, timestamp, person_id FROM events WHERE DATE(timestamp) >= ?", (start_week.isoformat(),))
    data = cur.fetchall()
    conn.close()

    if not data:
        story.append(Paragraph("No se registraron eventos en la semana.", normal_style))
    else:
        ...
        story.append(Paragraph("Tabla 1. Eventos de la semana", normal_style))
